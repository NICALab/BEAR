...
def parse(mode=None):
    ...
    ap.add_argument('--constrain_S', type=str, default=None,
                    ...
                    P if positive, N if negative.')
    ap.add_argument('--delta', type=float, default=0,
                    help='Thresholding on sparse matrix S. \
                    For more detail, go to our paper.')
    
    ap.add_argument('--sparse_NMF', type=float)
    ...
    if mode == None:
        ...
        ap.add_argument('--online_lr', type=float, default = 4e-5,
                        help='learning rate for online update')
        ap.add_argument('--window_length', type=int, default=4,
                        help='window length')
    else:
        raise AssertionError

    print("options_ipynb mode")
    args = ap.parse_args("")
    all_defaults = {}
    for key in vars(args):
        all_defaults[key] = ap.get_default(key)
